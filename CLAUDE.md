...
- `exclusion_reason(path, ext, size)` - the disc/BIOS guard. `.bin` is ambiguous (Sega Genesis/Atari 2600 carts *and* PS1/Saturn/Dreamcast/PC Engine CD disc images *and* BIOS). This refuses a `.bin` that sits in a disc-system folder (`DISC_SYSTEM_FOLDERS`), sits beside a `.cue`/`.gdi`/... descriptor (`_dir_has_disc_descriptor`, cached per dir), or exceeds `CARTRIDGE_BIN_MAX_BYTES` (16 MB). Any file in a `bios` folder is refused regardless of extension. Both scan paths call this; refused files go to `skipped_files` with a reason. **This guard is load-bearing safety — do not weaken it; `.bin` must never be blanket-compressed.**
